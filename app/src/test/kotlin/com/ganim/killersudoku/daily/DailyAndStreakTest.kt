...
class DailyAndStreakTest {
    ...
    inner class Schedule {
        ...
        @Test
        fun `the week builds to Saturday and eases off on Sunday`() {
            fun specOn(day: DayOfWeek) = DailySchedule.specFor(LocalDate.of(2026, 1, 5).with(day))

            specOn(DayOfWeek.MONDAY) shouldBe DailySpec(Difficulty.EASY)
            specOn(DayOfWeek.TUESDAY) shouldBe DailySpec(Difficulty.EASY)
            specOn(DayOfWeek.WEDNESDAY) shouldBe DailySpec(Difficulty.MEDIUM)
            specOn(DayOfWeek.THURSDAY) shouldBe DailySpec(Difficulty.MEDIUM)
            specOn(DayOfWeek.FRIDAY) shouldBe DailySpec(Difficulty.HARD)
            specOn(DayOfWeek.SATURDAY) shouldBe DailySpec(Difficulty.EXPERT)
            specOn(DayOfWeek.SUNDAY) shouldBe DailySpec(Difficulty.MEDIUM)
        }

        @Test
        fun `every difficulty appears, so every pool is used`() {
            DailySchedule.allSpecs.map { it.difficulty }.toSet() shouldBe Difficulty.entries.toSet()
        }

        @Test
        fun `days sharing a pool are grouped together`() {
            DailySchedule.weekdaysSharing(DayOfWeek.SUNDAY) shouldBe
                listOf(DayOfWeek.WEDNESDAY, DayOfWeek.THURSDAY, DayOfWeek.SUNDAY)
            DailySchedule.weekdaysSharing(DayOfWeek.SATURDAY) shouldBe listOf(DayOfWeek.SATURDAY)
        }
    }
