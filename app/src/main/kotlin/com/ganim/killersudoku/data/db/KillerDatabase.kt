package com.ganim.killersudoku.data.db
...
@Database(
    ...
    version = KillerDatabase.VERSION,
    exportSchema = true,
)
...
abstract class KillerDatabase : RoomDatabase() {
    ...
    companion object {
        const val VERSION = 1
        const val NAME = "killer.db"

        @Volatile
        private var instance: KillerDatabase? = null

        fun get(context: Context): KillerDatabase =
            instance ?: synchronized(this) {
                instance ?: build(context.applicationContext).also { instance = it }
            }

        private fun build(context: Context): KillerDatabase =
            Room.databaseBuilder(context, KillerDatabase::class.java, NAME).build()
    }
}
