...
data class Settings(
    ...
    /**
     * Auto-candidates (build plan 6): the app fills and maintains pencil marks. Many
     * players want it and purists do not, so it is off by default and remembered once set.
     */
...
class SettingsRepository(private val context: Context) {
    ...
    val settings: Flow<Settings> = context.settingsDataStore.data
        ...
        .map { prefs ->
            Settings(
                ...
                autoNotes = prefs[AUTO_NOTES] ?: false,
            )
        }
    ...
    suspend fun setAutoNotes(enabled: Boolean) {
        context.settingsDataStore.edit { it[AUTO_NOTES] = enabled }
    }
